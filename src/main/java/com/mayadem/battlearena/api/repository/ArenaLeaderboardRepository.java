...
public interface ArenaLeaderboardRepository extends JpaRepository<ArenaLeaderboard, Long> {
    ...
    // En iyi 100 oyuncu
    @Query("SELECT a FROM ArenaLeaderboard a ORDER BY a.rankPoints DESC LIMIT 100")
    List<ArenaLeaderboard> findTop100();
    ...
    @Query("SELECT new com.mayadem.battlearena.api.dto.LeaderboardStatsDto( COUNT(a), AVG(a.rankPoints), MAX(a.rankPoints), MAX(a.username) ) FROM ArenaLeaderboard a")
    LeaderboardStatsDto findGlobalStats();

    // Rank aralığındaki oyuncular
    @Query("SELECT a FROM ArenaLeaderboard a WHERE a.rankPosition BETWEEN :startRank AND :endRank ORDER BY a.rankPosition ASC")
